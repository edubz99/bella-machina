...
<div class="container">
    ...
    <form method="post" action="" name="insert_vehicle" id="insert_vehicle" enctype="multipart/form-data">
        ...
        <div class="form-group">
            ...
            <input type="text" class="form-control" id="title" placeholder="Enter title" name="title" value="<?php echo $post_to_edit->post_title;?>">
        </div>
        <div class="excerpt">
            <textarea name="car_excerpt" rows="5" placeholder="Excerpt"><?php echo $post_to_edit->post_excerpt;?></textarea>
        </div>
        ...
        <div class="row group-input">
                ...
                if($vehicle_makes ){
                    ...
                    <select name="vehicle_type" class="submit-vehicle-type" required>
                        ...
                        foreach ($vehicle_makes as $cat){
                            if($type ==$cat->term_id){
                                ?>
                                <option selected data-type="<?php echo esc_attr($cat->slug);?>" value="<?php echo esc_attr($cat->slug);?>"><?php echo esc_attr($cat->name); ?></option>
                                <?php
                            }else{
                            ?>
                ...
                if($vehicle_makes ){
                    ...
                    <select name="make" class="submit-vehicle-make" required>
                        ...
                        foreach ($vehicle_makes as $make){
                            ...
                            if($mk ==$make->term_id){
                                ?>
                                <option selected data-type ="<?php echo esc_attr($car_type);?>" data-make="<?php echo esc_attr($make->slug);?>" value="<?php echo esc_attr($make->slug);?>"><?php echo esc_attr($make->name); ?></option>
                                <?php
                            }else{
                            ?>
                ...
                if($vehicle_makes ){
                    ...
                    <select name="model" class="submit-vehicle-model" required>
                        ...
                        foreach ($vehicle_makes as $cat){
                            ...
                            if($mod ==$cat->term_id){
                                ?>
                                <option selected data-make="<?php echo esc_attr($car_type);?>" value="<?php echo esc_attr($cat->slug);?>"><?php echo esc_attr($cat->name); ?></option>
                                <?php
                            }else{
                            ?>
            ...
            <div class="vehicle-info vehicle-price col-md-3">
                ...
                <input type="number" value="<?php echo $price;?>" placeholder="insert your price example:50000" name="vehicle_price"/>
        ...
        <div class="vehicle-spec">
            <div class="vehicle-info vehicle-price">
                ...
                <input type="text" value="<?php echo $pricetxt;?>" placeholder="insert your text price example: Contact" name="vehicle_price_text"/>
            </div>
            <?php
            global $car_dealer;
            if ( is_plugin_active( 'progression-car-dealer-master/progression-car-dealer.php' ) ) {
                ...
                foreach ( $fields as $k => $field ) {
                    ...
                    if ( 'number' === $fieldtype ) {
                        ...
                        if($num_val){
                            $field_html .= '<input name="'. $field['name'] .'" value="'.$num_val.'" type="number"/> ';
                        }else{
                    ...
                    if ( 'radio' === $fieldtype || 'checkbox' === $fieldtype ) {
                        ...
                        foreach ( $field['choices'] as $choice_value => $choice_label ) {
                            if($d==1) {
					            ...
					            <input type="radio" '.$selected.' name="' . $field['name'] . '" value="' . $choice_value . '" required><label class="input-style"></label><span class="check"></span> ' . $choice_label . ' </label><br>';
                            }else{
                                if($ra_val == $choice_value){
                                    $selected ='checked';
                                }else{
                                ...
                                $field_html .= '<label class="choice choice-' . $choice_value . '">                            
					            <input type="radio" '.$selected.' name="' . $field['name'] . '" value="' . $choice_value . '"><label class="input-style"></label><span class="check"></span> ' . $choice_label . ' </label><br>';
                            }
                            $d++;
                        }
                        $field_html .= '</p>';
        ...
        <div class="row features-bottom">
            <div class="vehicle-feature feature_image_pre col-md-4">
                ...
                <div class="fileUpload">
                    ...
                    <input id="uploadBtn" type="file"  value="<?php echo esc_url($feature_img);?>" id="featuredimage" name="feature_image" accept="image/*">
                </div>
                <?php
                if($feature_img){
                    ...
                    <div class="img_pre"  data-id="<?php echo $carID; ?>">
                        ...
                        <span data-id="<?php echo $carID; ?>" class="img_delete"><i class="fa fa-times-circle"></i> </span>
                    </div>
                ...
                if($autoshowroom_gallery[0]){
                    foreach ($autoshowroom_gallery[0] as $image) {?>
                        <div class="gallery_pre_item" data-id="<?php echo $image;?>">
                           <img src="<?php echo esc_url(wp_get_attachment_url($image)); ?>" alt="<?php echo esc_attr(get_the_title($carID)); ?>"/>
