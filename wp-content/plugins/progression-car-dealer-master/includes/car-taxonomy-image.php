...
function car_dealer_edit_taxonomy_field($taxonomy) {
    ...
    if($taxonomy->taxonomy == 'make'){
        ...
        if(is_array($vehicle_type)){
            $make_id = $vehicle_type->term_id;
        }else{
            $make_id =$vehicle_type;
        }
        if($terms){
            $term_arr = '<select name="vehicle_type" class="vehicle_type" >';
            foreach ($terms as $term){
                if($term->term_id==$make_id){
...
//            $make_term = get_term_by('id',$makes,'make');
        ...
        if($terms_make){
            $terms_make_arr = '<select name="make" class="make">';
            $terms_make_arr .= '<option>'.$makeids.'o9</option>';
            foreach ($terms_make as $make){
                if(isset($makes) && $make->term_id==$makeids){
...
function car_dealer_save_taxonomy_image($term_id) {
    ...
    if(isset($_POST['vehicle_type']))
        update_field( 'vehicle_type',$_POST['vehicle_type'],'make_'. $term_id );

    if(isset($_POST['make']))

        update_field( 'make',$_POST['make'],'model_'. $term_id );
}
