...
class WIS_InstagramSlider extends WP_Widget {
	...
	public function public_enqueue() {
		...
		wp_enqueue_style( WIS_Plugin::app()->getPrefix() . 'instag-slider', WIS_PLUGIN_DIR.'/assets/css/instag-slider.css', array(), WIS_Plugin::app()->getPluginVersion() );
		wp_enqueue_script( WIS_Plugin::app()->getPrefix() . 'jquery-pllexi-slider', WIS_PLUGIN_DIR.'/assets/js/jquery.flexslider-min.js', array( 'jquery' ), '2.2', true );
	}
	...
	private function get_template( $template, $args ) {
		...
		if( $template == 'slider' ) {
			...
			if ( is_array( $args['description'] ) && count( $args['description'] ) >= 1 ) {
				...
				$output .= "<div class='jr-insta-datacontainer' style='display: none;'>\n";
