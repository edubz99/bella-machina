...
//            $field_name = $field['name'];
			...
			if ( 'radio' === $fieldtype || 'checkbox' === $fieldtype ) {
				...
				foreach ( $field['choices'] as $choice_value => $choice_label ) {
					...
					<input type="checkbox" name="'. $field['name'] .'[]" value="'. $choice_value .'" '. $checked .'> '. $choice_label. ' ('. $counter .')' .'</label><br>';
				}
				$field_html .= '</p>';
...
$meta_query = array(
	...
	public function get_dropdown_options( $field, $range, $min, $max, $selected ) {
		$output = '';
		$prepend 		= $field['prepend'] ? $field['prepend']. '&#8201' : '';
		$append 		= $field['append'] ? '&#8201' .$field['append'] : '';
...
class pcd_Walker_CategoryDropdown extends Walker {
    ...
    function start_el( &$output, $category, $depth = 0, $args = array(), $id = 0 ) {
        ...
        if ( 'make' == $category->taxonomy ) {
            ...
            if($make1){
                if(is_object($make1)){
                    ...
                    $term = get_term( $make1, 'vehicle_type' );
                    $car_type =$term->slug;
                }
            }
            ...
            if(is_object($make)){
                ...
                $make_attr ='data-make="'. $make_term->slug .'"';
            }
            if ( empty( $make ) ) {
                return;
            }
        ...
        if ( 'make' == $category->taxonomy ) {
            ...
            $output .= "\t<option ".$make_attr." class=\"level-$depth\" value=\"" . $category->slug . "\"";
        }
