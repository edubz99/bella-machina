...
function autoshowroom_vehicle_slider( $atts )
{
    ...
    extract(shortcode_atts(array(
        ...
        'autoshowroom_vehicle_carousel_number'          =>  1,
            ...
            <?php while ( $vehicles->have_posts() ) : $vehicles->the_post();
                ...
                <div class="item">
                    <div class="Vehicle-Feature-Image <?php echo esc_attr($height_screen);?>"
                         style="height:<?php echo esc_attr($autoshowroom_vehicle_carousel_height);?>px; background:url(<?php echo esc_attr($url); ?>) center no-repeat">
                    ...
                    <?php if($autoshowroom_vehicle_title=='show'){ ?>
                        <h4 class="Vehicle-Title container" style="top:<?php echo esc_attr($autoshowroom_vehicle_carousel_title_top);?>px;">
                            <a href="<?php echo get_permalink(); ?>"><?php the_title(); ?></a>
                            <span>
                            <label><?php esc_html_e('MSRP FROM','tz-autoshowroom'); ?></label>
                                <?php
                                echo tz_autoshowroom_filter_vehicle_msrp_price(get_the_ID(),$showmsrp);
                                ?>
                        </span>
                        </h4>
                    <?php } ?>
                            ...
                            <div class="vehicle-slider-des container">
                                <div class="slider-info">
                                    ...
                                    <?php if($autoshowroom_vehicle_description_limit){ ?>
                                        <p><?php echo substr(strip_tags(do_shortcode('[vehicle_description]')), 0, $autoshowroom_vehicle_description_limit);?></p>
                                    <?php } else{
